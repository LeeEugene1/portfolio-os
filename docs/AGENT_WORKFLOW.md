...
- 이슈 PR은 사용자 실행 방식, 검증 방식, 배포 절차, 로컬 하네스 절차가 바뀌면 `README.md`를 함께 업데이트합니다.
- 플랜의 "변경 예정 파일"에 `README.md` 반영 여부를 포함합니다.
- README 변경이 필요 없다고 판단하면 PR 본문의 참고 항목에 그 이유를 남깁니다.
