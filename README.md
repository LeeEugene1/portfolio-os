...
웹앱과 Flutter WebView 기반 모바일 앱을 하나의 모노레포로 구성했습니다. 같은 포트폴리오 경험을 공유하기 때문에 코드와 문서를 한 곳에서 관리해 유지보수 편의성을 높였습니다. 또한 이후 모바일에서 뒤로가기 동작, 로딩 UI, 에러 처리 같은 WebView 세부 기능을 추가할 때도 기존 웹앱의 맥락과 문서를 함께 참고할 수 있도록 설계했습니다.
...
## 하네스
  ...
  병렬 에이전트 작업에서는 구현 자체보다 실행 환경을 일관되게 유지하는 것이 중요했습니다. 그래서 로컬 서버 실행, 포트 선택, 검증 명령, 서버 종료 기준을 하네스로 문서화 했습니다.
...
여러 Codex 세션을 동시에 실행하면서 몇가지 문제를 겪었고 이를 해결하였습니다.
